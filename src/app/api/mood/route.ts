...
export async function POST(request: NextRequest) {
  try {
    ...
    let savedEntry = null;
    if (db) {
      const result = await db.insert(schema.moodEntries).values(moodEntry).returning();
      savedEntry = result[0];
    } else {
    ...
    return NextResponse.json({ success: false, error: 'Failed to process mood submission' }, { status: 500 });
  }
}
